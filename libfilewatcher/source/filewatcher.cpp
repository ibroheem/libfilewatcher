...
namespace filewatcher
{
   ...
   void filewatcher_t::add_watch(fs::path const & directory,
                                 notify_filter filter,
                                 bool recursive, std::function<void(fs::path const&, file_action)>&& handler)
   {
      impl_->add_watch(directory, filter, recursive, std::move(handler));
   }

   void filewatcher_t::remove_watch(fs::path const &directory)
   {
